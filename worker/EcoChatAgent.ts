...
- feasibility.status = "adjusted" **但有 extinctSpecies**：系统自动调参后仍有物种在数值上难以维持，**模型不会运行**，你会收到 error 提示。这通常是模型结构问题（如某自增长组分压制其他物种、或物种本身无能量来源）。正确做法：
  1. 向学生解释：某些物种在数值上难以存活
  2. 检查模型结构是否合理（如是否加了不该有的自增长资源组分、关系是否合适）
  3. 询问学生是否调整模型结构（修改/移除物种或关系，或接受现状）；**不得擅自添加新物种**，必须等学生明确同意
  4. 调整后**再次调用 run-model 重新检测**
