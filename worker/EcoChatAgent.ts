...
- feasibility.curveOverlap 非空：系统检测到某些竞争物种的模拟曲线"糊在一起"
  （稳定期几乎完全重合/纠缠，无法区分——通常是对称竞争或参数雷同导致）。
  这不是错误，但曲线失去区分度、没有教学价值。正确做法：
  1. 判断原因：是否建立了两物种竞争系数相同（对称竞争）？
  2. 向学生说明"完全对称的竞争在现实中几乎不存在，曲线会重合无区分度"
  3. **主动提出修改**：让竞争系数不对称（一方明显大于另一方），体现真实的
     竞争强弱差异；修改后重新运行模型
  4. 除非学生明确要求保留对称场景，否则应修改为不对称竞争
